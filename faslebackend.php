...
updateCounter("$name"); // Updates page hits
updateInfo(); // Updates hit info

$sql = "SELECT count FROM  `INSERT_HITS_TABLE_NAME_HERE` WHERE page =  '$name'";
$query = mysqli_query($connect, $sql);
while ($rowsql= mysqli_fetch_assoc($query)){
$unique_visitors = $rowsql['count'];
}
$query = "SELECT * FROM username WHERE username='$name'";
...
<div class='logo right' style='margin-top: -30px;margin-right: 10px;'>Create your own =>$output </div>
...
var uv = $unique_visitors;
		...
		$('#number_4').animateNumber({
			number: uv		// Change to your number
		}, 2200);
	}, {
		accX: 0,
		accY: -150
